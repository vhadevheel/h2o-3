...
test.model.gam.early.stop <- function() {
    ...
    expect_true(length(gam_early_stop@model$glm_scoring_history_early_stop) > 0, "Early stop is not working")
    gam_lambda_search <- h2o.gam(y = "C21", x = c(1:20), gam_columns = c("C11"), training_frame = train, validation_frame = valid,
            family = "gaussian", stopping_rounds=3, stopping_metric="rmse", stopping_tolerance=0.1, lambda_search=TRUE, nlambdas=3)
    expect_true(is.null(gam_lambda_search@model$glm_scoring_history_early_stop), "Early stop should have been disabled for lambda_search=TRUE but is not")
