...
def test_gam_model_predict():
    ...
    for ind in range(len(early_stop_metrics)):
        ...
        assert len(h2o_model._model_json["output"]["glm_scoring_history_early_stop"].cell_values) <= numIter, \
            "Number of iterations without early stop: {0} should be more than with early stop: " \
            "{1}".format(numIter, len(h2o_model._model_json["output"]["glm_scoring_history_early_stop"].cell_values))
